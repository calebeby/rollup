...
export default function umd(
	...
	{ graph, exportMode, indentString: t, intro, outro, dependencies, exports }: FinaliserOptions,
	...
	const cjsExport = exportMode === 'default' ? `module.exports${_}=${_}` : ``;
	const defaultExport =
		exportMode === 'default'
			? `${setupNamespace(options.name, 'global', true, options.globals, options.compact)}${_}=${_}`
			: '';
	...
	const exportBlock = getExportBlock(exports, dependencies, exportMode, options.interop, options.compact);
